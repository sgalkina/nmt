...
#             vh = tuple(hidden_channels) + (in_channel // 2,)
       ...
       # Convolutional branch for the first tensor
        ...
        self.combined_fc = nn.Sequential(
            ...
            nn.ReLU(),
            nn.BatchNorm1d(128),
            nn.Linear(128, 36 * 3 * 3),  # Output size matching the desired output shape flattened
            nn.ReLU()
        )
        
        # Final layer to reshape the output to (batch_size, 9, 3, 3)
        self.output_layer = nn.Sequential(
            nn.Unflatten(1, (36, 3, 3))
        )
...
class GraphAffineCoupling(nn.Module):
    def __init__(self, n_node, in_dim, hidden_dim_dict, masked_row, affine=True):
        ...
        self.conv_branch = nn.Sequential(
            nn.Conv1d(in_channels=9, out_channels=32, kernel_size=3, padding=1),  # 1D convolution
            nn.ReLU(),
            nn.BatchNorm1d(32),
        ...
        # Combined fully connected layers
        self.combined_fc = nn.Sequential(
            nn.Linear(288, 256),  # input size is the sum of the two branches
            nn.ReLU(),
            nn.BatchNorm1d(256),
            nn.Linear(256, 9*10),  # Output size matching the desired output shape flattened
            nn.ReLU()
        )
